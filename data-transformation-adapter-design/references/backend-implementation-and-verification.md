...
- For source-query-simple APIs, put only source-to-target aliases, type serialization, null behavior, masking, and response-envelope assembly in service/serializer modules. Do not add aggregation, business formulas, totals, rankings, or chart/KPI derivation.
- When the authoritative source is a database and the endpoint is source-query-simple, push eligible filtering, sorting, pagination, and permission scope into SQL/database views/repository queries. Do not add joins, grouping, aggregation, Top/Bottom, exact counts, or formulas unless there is an approved derived/summary exception.
- For approved derived/summary endpoints, push eligible filtering, sorting, pagination, joins, grouping, aggregation, Top/Bottom, and count calculations into SQL/database views/repository queries. Do not pull broad database rows into memory for business filtering or aggregation.
- Do not use a full-materialize-then-filter pipeline for global/page-level scope. Source/repository code must apply global request filters, permission scope, sorting, and pagination before service-layer response shaping. Building a complete target list and trimming it later is a fail unless the input is a documented component-internal filter over already fetched component data, tiny static enum, or bounded lookup.
...
- SQLite simulation must be experience-equivalent to the real API for parameter behavior. Seed fewer rows if necessary, but include enough combinations for filters, date ranges, statuses, permissions, sorting, pagination, drilldown, empty, and abnormal scenarios to produce visibly different responses. Add Top/Bottom or aggregate scenarios only for approved derived/summary endpoints.
...
- For source-query-simple APIs, service-layer transformations must not change row grain or compute aggregate business meaning.
...
- Scenario:
- Request params:
- Table-content evidence inspected:
...
- Minimal implementation mode and exception status:
...
- For source-query-simple APIs, add focused tests or review samples proving request filters map to source predicates and that retrieval has no hidden aggregation/processing.
- Add focused tests for non-trivial field renames, date/period conversion, enum mapping, unit conversion, rounding, default fill, sorting, and empty states. Add aggregation/formula tests only for approved derived/summary endpoints.
- For SQLite simulation, add parameter-variation tests proving that filters, time ranges, dimensions, permission scope, sorting, pagination, and drilldown requests change the response according to the API contract. Add aggregate requests only for approved derived/summary endpoints.
...
- For collection/list/table transformations, include pagination boundary tests for default page size, maximum page size, out-of-range pages, and stable ordering. Exact total-count behavior requires an explicit need and should not be assumed for source-query-simple APIs.
