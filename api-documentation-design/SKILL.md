...
description: "[后端阶段] 本阶段版本仅服务后端/数据服务设计、API文档/契约、接口实现、运行验证和后端交付；不承接原型设计、前端代码或测试执行。用于生成或重构可交付的API/接口文档。用户提到接口文档、API文档、接口说明、OpenAPI/Swagger、前后端接口约定、请求响应示例、鉴权/错误码/分页/筛选、快照接口、snapshotDate/dataVersion/loadBatch、字段口径、响应字段兼容、新增字段命名、默认后端技术栈、Python/Flask/连接池/Redis、已有路由补文档、接口交付验收时触发；只写文档契约，不实现后端代码或前端接入。"
---
