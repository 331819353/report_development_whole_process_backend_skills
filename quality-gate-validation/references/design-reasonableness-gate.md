...
11. API/backend feasibility.
   API endpoints, response models, data models, transformations, auth, pagination, sorting, exports, and error states can support the UI contract without hidden invention. For table-backed interfaces, every involved table/view/fixture/upstream object has table-content evidence, and every client-visible filter is a request param mapped to a source predicate. Global/page-level filters and permission scope execute through SQL `WHERE`, source/provider/repository queries, resolver params, Redis/precompute keys, or equivalent source-side scope wherever feasible; component-internal filters may operate on already fetched component data. A design that lacks table-content evidence, hides filters outside request params, or depends on page/API-level full-materialize-then-filter behavior is a `DESIGN-*` finding unless explicitly bounded.
...
12. Report data-service backend fit.
   ...
   Simple table retrieval APIs should be source-query-simple: projection, request-param predicates, backend-injected scope, stable order, pagination, and serializer aliases. A design that hides joins, aggregation, exact counts, formulas, totals, rankings, chart/KPI derivation, or broad response reshaping inside a simple endpoint is a `DESIGN-*` finding unless it is documented as a derived/summary/precompute exception.
...
14. SQL query reasonableness.
   Database-backed APIs have reasonable query shape. For source-query-simple APIs: only needed columns are selected, predicates are source-side, sargable, and mapped from request params, pagination is bounded and stable, optional filters avoid broad nullable-OR templates, and joins/aggregation/exact counts/formulas/totals/rankings are absent. For derived/summary exceptions: join keys/cardinality are complete, one-to-many multiplication is controlled, dedup/order operations are intentional, aggregation/window work happens after early filters when possible, and risky P0/high-volume queries have plan evidence or a slow-query gap. A design that depends on `SELECT *`, broad in-memory filtering, incomplete joins, hidden aggregation, deep unbounded pagination, or large unverified sorts is a `DESIGN-*` finding.
...
- Table-backed API work has table-content evidence before implementation: row grain, keys, filter fields, representative samples, permission fields, and result bounds.
- Global filter/query behavior is feasible through request-param predicates without building or fetching all candidate data before applying scope; component-internal filters are explicitly local to already fetched component data.
- Simple source-query APIs do not hide joins, aggregation, exact counts, formulas, totals, rankings, Top/Bottom, or broad service-side processing.
