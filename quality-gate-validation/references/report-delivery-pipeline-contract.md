...
- Minimal interface implementation evidence when backend/API implementation is in scope: table-content understanding for every involved table/view/fixture/upstream object, request-param-to-source-field mapping, source-query-simple vs derived-summary-exception status, query-only boundary proof, and open `GAP-*`.
